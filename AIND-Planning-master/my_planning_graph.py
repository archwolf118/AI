...
class PlanningGraph():
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for e1 in node_s1.parents:
            for e2 in node_s2.parents:
                if not e1.is_mutex(e2):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            for l_index, node_s_set in enumerate(self.s_levels):
                node_s = PgNode_s(g, True)
                if node_s in node_s_set:
                    level_sum = level_sum + l_index
                    break
